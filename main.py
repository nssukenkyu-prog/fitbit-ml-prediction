"""
Fitbit睡眠最適化ML予測サービス (v3.1 - モデルキャッシュ対応版)
Render.comで24時間稼働するFlaskアプリケーション

変更点:
- v3.1: モデルキャッシュ機能を追加 (USER_MODEL_CACHE)
"""
...
def get_or_train_model(df, user_sheet_name):
    ...
    USER_MODEL_CACHE[user_sheet_name] = {
        'model': model,
        'features': features,
        'avg_features': avg_features_for_pred,
        'timestamp': now
    }
...
def process_prediction_queue(ss):
    ...
    print("\\n" + "="*70)
    print("🔄 ML予測キュー処理を開始します (v3.1: モデルキャッシュ対応)")
    print("="*70)
...
def trigger_prediction():
    """
    Cloud Scheduler または GAS からのトリガーで実行
    """
    try:
