"""
Fitbit睡眠最適化ML予測サービス (v3.1.1 - 修正完了版)
Render.comで24時間稼働するFlaskアプリケーション

修正履歴:
- v3.0: 特徴量エンジニアリング追加
- v3.1: モデルキャッシュ機能追加
- v3.1.1: Plan Bの日付計算バグ修正 & スプレッドシート列名の揺らぎ吸収
"""
...
def preprocess_data(ss, user_sheet_name):
    ...
    # 【重要】ヘッダーの余計な空白を削除
    sleep_df.columns = [c.strip() for c in sleep_df.columns]

    # 【重要】列名の表記ゆれを吸収
    rename_map = {
        'deepMinutes': 'deep.minutes',
        'remMinutes': 'rem.minutes',
        'lightMinutes': 'light.minutes',
        'wakeMinutes': 'wake.minutes',
        'wakeCount': 'wake.count',
        'minutesToFallAsleep': 'minutesToFallAsleep'
    }
    sleep_df = sleep_df.rename(columns=rename_map)

    # 必須カラムの存在確認
    if 'dateOfSleep' not in sleep_df.columns:
        print(f"  ⚠️ 'dateOfSleep' 列が見つかりません: {user_sheet_name}")
        return None

    # 日付変換
    sleep_df = sleep_df.drop_duplicates(subset=['dateOfSleep'], keep='last')
    sleep_df['dateOfSleep'] = pd.to_datetime(sleep_df['dateOfSleep'], errors='coerce')
    sleep_df = sleep_df.dropna(subset=['dateOfSleep'])
    ...
    # 就寝時刻の計算
    if 'startTime' in sleep_df.columns:
        sleep_df['startTime'] = pd.to_datetime(sleep_df['startTime'], errors='coerce')
        sleep_df = sleep_df.dropna(subset=['startTime'])
        base_hour = 4
    ...
    if 'minutesToFallAsleep' in sleep_df.columns:
        sleep_df['sleep_onset_speed'] = sleep_df['minutesToFallAsleep']
    else:
        sleep_df['sleep_onset_speed'] = 0
    
    if 'minutesAsleep' in sleep_df.columns:
        ...
        for idx in sleep_df.index:
            if sleep_df.loc[idx, 'minutesAsleep'] < 420:
                deficit_count += 1
            else:
                deficit_count = 0
            sleep_df.loc[idx, 'sleep_deficit_streak'] = deficit_count

    if 'bedtime_minutes' in sleep_df.columns:
    ...
    if hrv_df is not None and not hrv_df.empty:
        hrv_df.columns = [c.strip() for c in hrv_df.columns]
        hrv_df = hrv_df.drop_duplicates(subset=['date'], keep='last')
        hrv_df['date'] = pd.to_datetime(hrv_df['date'])
        if 'dailyRmssd' in hrv_df.columns:
            sleep_df = pd.merge(sleep_df, hrv_df[['date', 'dailyRmssd']], left_on='dateOfSleep', right_on='date', how='left')
    ...
    if rhr_df is not None and not rhr_df.empty:
        rhr_df.columns = [c.strip() for c in rhr_df.columns]
        rhr_df = rhr_df.drop_duplicates(subset=['date'], keep='last')
        rhr_df['date'] = pd.to_datetime(rhr_df['date'])
        if 'restingHeartRate' in rhr_df.columns:
            sleep_df = pd.merge(sleep_df, rhr_df[['date', 'restingHeartRate']], left_on='dateOfSleep', right_on='date', how='left')
            sleep_df = sleep_df.drop(columns=['date'])
    ...
    if spo2_df is not None and not spo2_df.empty:
        spo2_df.columns = [c.strip() for c in spo2_df.columns]
        spo2_df = spo2_df.drop_duplicates(subset=['date'], keep='last')
        spo2_df['date'] = pd.to_datetime(spo2_df['date'])
        if 'spo2_avg' in spo2_df.columns:
            sleep_df = pd.merge(sleep_df, spo2_df[['date', 'spo2_avg']], left_on='dateOfSleep', right_on='date', how='left')
            sleep_df = sleep_df.drop(columns=['date'])
    ...
    if br_df is not None and not br_df.empty:
        ...
        if 'breathingRate' in br_df.columns:
            sleep_df = pd.merge(sleep_df, br_df[['date', 'breathingRate']], left_on='dateOfSleep', right_on='date', how='left')
            sleep_df = sleep_df.drop(columns=['date'])
    ...
    if temp_df is not None and not temp_df.empty:
        ...
        if 'tempVariation' in temp_df.columns:
            sleep_df = pd.merge(sleep_df, temp_df[['date', 'tempVariation']], left_on='dateOfSleep', right_on='date', how='left')
            sleep_df = sleep_df.drop(columns=['date'])
...
def calculate_recovery_score(df, today_hrv, today_rhr):
    ...
    if score_val > 65: return "良好"
    elif score_val < 35: return "注意"
    else: return "安定"

def analyze_trends(df):
    if len(df) < 7: return "安定", "安定"
    hrv_trend_val = 0
    deep_trend_val = 0
    
    hrv_7day_avg = df['dailyRmssd'].rolling(window=7).mean().dropna()
    if len(hrv_7day_avg) >= 2:
        hrv_trend_val = hrv_7day_avg.iloc[-1] - hrv_7day_avg.iloc[-2]
        
    deep_7day_avg = df['deep.minutes'].rolling(window=7).mean().dropna()
    if len(deep_7day_avg) >= 2:
        deep_trend_val = deep_7day_avg.iloc[-1] - deep_7day_avg.iloc[-2]

    if hrv_trend_val > 2: trend_hrv = "上昇傾向 (良い兆候)"
    elif hrv_trend_val < -2: trend_hrv = "下降傾向 (要注意)"
    else: trend_hrv = "安定"

    if deep_trend_val > 5: trend_deep = "上昇傾向"
    elif deep_trend_val < -5: trend_deep = "減少傾向"
    else: trend_deep = "安定"
...
def get_key_factor(model, features):
    ...
    feature_names = {
        'bedtime_minutes': '就寝時刻のズレ', 'timeInBed': 'ベッドにいた時間',
        'day_of_week': '曜日', 'is_weekend': '週末', 'month': '季節',
        'deep_ratio': '深睡眠の比率', 'rem_ratio': 'レム睡眠の比率', 'wake_severity': '中途覚醒の深刻度',
        'sleep_onset_speed': '入眠速度', 'sleep_3day_avg': '3日間平均睡眠時間', 'sleep_7day_std': '睡眠時間の安定性',
        'sleep_diff_prev_day': '前日との睡眠時間差', 'bedtime_7day_std': '就寝時刻の安定性', 'sleep_deficit_streak': '連続睡眠不足日数',
        'dailyRmssd': '心拍変動(HRV)', 'hrv_7day_avg': 'HRVの7日間平均', 'hrv_diff_prev_day': 'HRVの前日比',
        'restingHeartRate': '安静時心拍数(RHR)', 'rhr_7day_avg': 'RHRの7日間平均',
        'spo2_avg': '睡眠中の平均SpO2', 'breathingRate': '睡眠中の呼吸数', 'tempVariation': '皮膚温の変化'
    }
...
def simulate_plan_b(model, features, avg_features_for_pred, best_bedtime, target_date_str):
    ...
    if 'day_of_week' in features: search_df['day_of_week'] = tomorrow_dow
    if 'is_weekend' in features: search_df['is_weekend'] = tomorrow_is_weekend
    if 'month' in features: search_df['month'] = tomorrow_month
...
def get_or_train_model(df, user_sheet_name):
    global USER_MODEL_CACHE
    now = datetime.now()
    
    if user_sheet_name in USER_MODEL_CACHE:
        cache_entry = USER_MODEL_CACHE[user_sheet_name]
        if (now - cache_entry['timestamp']) < timedelta(hours=CACHE_EXPIRATION_HOURS):
            print(f"  ℹ️ キャッシュされたモデルを使用 (Last trained: {cache_entry['timestamp']})")
            return cache_entry['model'], cache_entry['features'], cache_entry['avg_features']
        else:
            print(f"  ℹ️ モデルキャッシュ期限切れ。再学習します。")
    
    print(f"  🤖 機械学習モデルを学習中... ({len(df)}日分のデータ)")
    
    feature_candidates = [
        'bedtime_minutes', 'timeInBed', 'day_of_week', 'is_weekend', 'month',
        'deep_ratio', 'rem_ratio', 'wake_severity', 'sleep_onset_speed',
        'sleep_3day_avg', 'sleep_7day_std', 'sleep_diff_prev_day', 'bedtime_7day_std', 'sleep_deficit_streak',
        'dailyRmssd', 'hrv_7day_avg', 'hrv_diff_prev_day',
        'restingHeartRate', 'rhr_7day_avg', 'spo2_avg', 'breathingRate', 'tempVariation'
    ]
    ...
    for feature in feature_candidates:
        if feature in df.columns:
            if feature in ['bedtime_minutes', 'timeInBed', 'day_of_week', 'is_weekend', 'month']:
    ...
    USER_MODEL_CACHE[user_sheet_name] = {
        'model': model, 'features': features, 'avg_features': avg_features_for_pred, 'timestamp': now
    }
    return model, features, avg_features_for_pred

def predict_for_single_user(ss, user_sheet_name, target_date_str):
    try:
        print(f"\n{'─'*70}\n🤖 予測処理: {user_sheet_name} - {target_date_str}\n{'─'*70}")
        ...
        if len(df) < MIN_DATA_DAYS:
            ...
            confidence = 'low'
            recovery_score, trend_hrv, trend_deep, key_factor = "安定", "データ収集中", "データ収集中", "データ収集中"
            plan_b_bedtime, plan_b_waketime = "N/A", "N/A"
        else:
            model, features, avg_features_for_pred = get_or_train_model(df, user_sheet_name)
            target_bedtime_str = get_user_preferences(ss, user_sheet_name)
            target_bedtime_minutes = convert_time_str_to_minutes(target_bedtime_str) if target_bedtime_str else None
            ...
            if target_bedtime_minutes is not None:
                ...
                bedtimes = np.arange(search_center - 60, search_center + 65, 5)
            else:
                print(f"  ℹ️ ユーザー希望時刻が未設定のため、標準範囲でグリッドを生成します。")
                bedtimes = np.arange(-180, 125, 5)
            
            times_in_bed = np.arange(360, 545, 5)
        ...
        for idx, row in enumerate(existing_data, start=2):
            if row.get('user_sheet_name') == user_sheet_name and row.get('date') == target_date_str:
                existing_row_index = idx
                break
        
        result_row = [
            target_date_str, user_sheet_name, pred_bedtime_str, pred_waketime_str,
            f"{best_quality:.1f}", confidence, recovery_score, trend_hrv, trend_deep,
            key_factor, plan_b_bedtime, plan_b_waketime
        ]
...
def process_prediction_queue(ss):
    ...
    for i, row in enumerate(all_values, start=2):
        if row[status_col - 1] == 'pending':
            target_row_index = i
            request = {'userSheetName': row[sheet_name_col - 1], 'targetDate': row[target_date_col - 1]}
            break

    if not request:
        print("✅ pendingステータスのリクエストはありません。")
        return {'success': True, 'message': 'pendingリクエストなし', 'processed': 0}

    user_sheet_name = request['userSheetName']
    target_date_str = request['targetDate']
    print(f"📋 処理対象: {user_sheet_name} @ {target_date_str} (行: {target_row_index})")
    ...
    try:
        ...
        if success:
            ...
            queue_sheet.update_cell(target_row_index, processed_at_col, datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            print(f"  ✅ 成功: 1件")
            ...
            print(f"  ❌ 失敗: 1件")
            return {'success': True, 'processed': 0, 'failed': 1}
    except Exception as e:
...
def home():
    return jsonify({'status': 'running', 'service': 'Fitbit ML Prediction API', 'version': '3.1.1'})
